/**
 * Silico Battles v17.1
 * 
 * Theme: Industry and automation
 * Project: Smart Energy Conservation in industrial application 
 * 
 * @author Salil Mittal (XI-E DPS RK PURAM)
 * @date 19th Oct 2021
 */ 
...
namespace Factory {
        ...
        ultrasonic{ultrasonic}, standby{standby}, monitor{monitor} 
    {
        ...
        pinMode(standby.btn_pin, INPUT);
        pinMode(standby.led_pin, OUTPUT);
        pinMode(standby.count_pin, OUTPUT);
        pinMode(standby.presence_led, OUTPUT);

        pinMode(monitor.ldr_in, INPUT);
        pinMode(monitor.ldr_out, INPUT);
        pinMode(monitor.tmp_in, INPUT);
        pinMode(monitor.tmp_out, INPUT);
    }
    ...
    struct LED_Set {
        LED_Set(int set_1, int set_2, int set_3): 
    ...
    Output::Output(int relay, Motor m_cooling, Motor m_process, LED_Set led_set):
        relay{relay}, m_cooling{m_cooling}, m_process{m_process}, led_set{led_set} {
            ...
            pinMode(m_cooling.in3, OUTPUT);
            pinMode(m_cooling.in4, OUTPUT);
            pinMode(m_cooling.pwm, OUTPUT);

            pinMode(m_process.in3, OUTPUT);
            pinMode(m_process.in4, OUTPUT);
            pinMode(m_process.pwm, OUTPUT);

            pinMode(led_set.set_1, OUTPUT);
            pinMode(led_set.set_2, OUTPUT);
            pinMode(led_set.set_3, OUTPUT);
    }
    ...
    void Output::adjust_lighting(int intensity) const {
        ...
        analogWrite(led_set.set_1, mapped);
        analogWrite(led_set.set_2, mapped);
        analogWrite(led_set.set_3, mapped);
    }
    ...
    void Output::adjust_cooling(int analog) const {
        ...
        digitalWrite(m_cooling.in3, LOW);
        digitalWrite(m_cooling.in4, HIGH);
        analogWrite(m_cooling.pwm, m_out);
    }

    void Output::run_processing_unit() const {
        digitalWrite(m_process.in3, LOW);
        digitalWrite(m_process.in4, HIGH);

        analogWrite(m_process.pwm, motor_high);
        delay(100);
    }

    /* Adjust lighting as HIGH or LOW */
    void Output::lighting_digital(bool state) const {
        digitalWrite(led_set.set_1, state);
        digitalWrite(led_set.set_2, state);
        digitalWrite(led_set.set_3, state);
    }
};
